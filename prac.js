...
function drawPillar(x, depth, gap=100, color="lime"){
...
function draw(){
	...
	for(var i=0; i<map.pillars.length; i++){
		...
		drawPillar(map.pillars[i].pos , map.pillars[i].depth, map.inGap, "lime");
		map.pillars[i].pos -= map.speed;
	}

	if(map.pillars[0].pos < 0 - map.width){
		map.pillars[0].pos = canvas.width;
		map.pillars[0].depth = Math.random()*(canvas.height-500)+200;
...
function reset(){
	...
	for(var i=0; i<map.numPillars; i++){
		...
		map.pillars[i].depth = Math.random()*(canvas.height-500)+200;
